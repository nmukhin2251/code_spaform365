...
//define(['text!./filebox.html','filebox-apppart', 'filebox-utils', 'filebox-handlelinks'], function( htmlString) {
define(['text!./filebox.xml', '../Runtime/source/syscomponents/filebox-apppart', '../Runtime/source/syscomponents/filebox-utils', '../Runtime/source/syscomponents/filebox-handlelinks'], function( htmlString) {
		...
		this.value = ko.observable({results:[]}).extend({ namespace: params });//this.value = ko.observable({results:[]}).inNamespace( params.namespace, params.InternalName);//.extend({ listItem: this.internalName });
		this.results = ko.observableArray([]);
				
		var map = spaform.form['_'].attachmentsmap;
				
		var previewFiles = function() {

			setTimeout(function(){
				window.Medline.HandleLinks.UpdateContentLinks('#formContainer');
			},500);
			/*
			var f = spaform.form;
			if(!f.attachments) {
				// attachments pages
				require(['runtime-attachment'], function(Attachments) {
					f.attachments = new Attachments();
					f.attachments.onSelected(function(attachment) {
						console.log(' ---> attachment ' + attachment.name + ' selected ')
						console.log(' ---> element ' + attachment.element.id + ' --')
					});
					//debugger;								
					f.attachments.get('TEST1');
					f.attachments.get('TEST2');
					
					//console.log(' $$$> element ' + f.attachments.get('TEST2').element.id + ' --')
					//console.log(' $$$> element ' + f.attachments.get('TEST1').element.id + ' --')
					//debugger;
					//f.attachments.get('TEST1').select();
					//debugger;	
					//f.attachments.get('TEST2').close();															
				});	
			};
			*/	
		};
		this.processValue = function(val) {
			if( !val) val = {};
			...
			if( params.PreviewAttachments == true) previewFiles();			
		};
		...
		this.addFile = function () {
			...
			spaform.list.item.upload().done( function(fileName) {
				var r = self.value();
				var item = { 
...
//	var fabric;
	...
	(function(){
		...
		this.$init = function(element) {
			...
			self.fabricObject["_filePickerIcon"] = self.fabricObject._container.querySelector(".ms-Icon--Attach");
			if( ko.validation) {
